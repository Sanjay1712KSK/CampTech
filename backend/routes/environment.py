...
@router.get('/environment/current', response_model=EnvironmentResponse)
def environment_current(params: CoordinatesQuery = Depends(), db: Session = Depends(get_db)):
    return environment(params=params, db=db)
...
def environment_override(payload: EnvironmentOverrideRequest):
    state = set_environment_override(
    ...
    return EnvironmentOverrideResponse.model_validate(state).model_dump()
